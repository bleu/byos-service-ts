...
That cap is an RPC budget as much as a memory bound. Batches go out back to back, so a tick makes `ceil(BALANCE_ACTIVE_SET_MAX / BALANCE_REFRESH_BATCH_SIZE)` requests — 200 at the shipped defaults. Raising the cap past roughly `BALANCE_REFRESH_INTERVAL_SECS / per-request latency * BALANCE_REFRESH_BATCH_SIZE` means the tick stops fitting inside its own interval, at which point the refresh interval silently becomes the tick duration and "clears within one refresh interval" stops being true. The job warns once the set passes half the cap, which is earlier than the trim firing — a firing trim already means legitimate sub-solvers are being evicted.

Balances are individual keys with a TTL, not fields in one hash. A hash needs the eviction sweep to delete the field as well as the sorted-set member, and missing that leaks an entry for the lifetime of the process; a key with a TTL expires whether or not anything remembers to clean it up. It also keeps the sweep atomic, since there is no second structure to read before writing. The TTL is the eviction age: while an address is in the refresh set the tick rewrites the key every interval, so the clock only starts once nothing is refreshing it. Serving a stale balance is bounded far tighter than the TTL in practice, because reading one re-enrols the address and the next tick corrects it.
