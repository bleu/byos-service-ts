...
- **Eager per-submission validation task (async, but validate immediately).** Originally rejected — two validation entry points to keep consistent for a latency win the polling loop doesn't need. **Partially reversed**: a bounded fire-and-forget simulation is now triggered after `POST /proposals` succeeds, so proposals are often already `active` by the time the driver polls `/solve`. The background tick remains the authoritative path: it runs `beginTick()` (which clears the escrow cache) and enforces `VALIDATION_CONCURRENCY=8`. The immediate path skips the cache clear, caps itself at 2 concurrent calls, and silently drops when at the limit — the background tick catches anything it misses within one tick interval. See `apps/byos/src/index.ts` for the semaphore and `routes.ts` for the call site.
