...
Status: accepted; revised 2026-08-14 (minBuyAmount/quotedBuyAmount split)
...
- `POST /proposals` — `{order_uid, sell_amount, min_buy_amount, quoted_buy_amount, interactions, valid_until, nonce, signature}`; token addresses come from the orderbook order, not the sub-solver ([ADR-0012](0012-simulation.md))
