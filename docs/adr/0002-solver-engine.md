...
Status: proposed; revised 2026-08-14 (minBuyAmount/quotedBuyAmount split)
...
CoW's score is surplus plus protocol fees; gas reaches it only because the solver declares gas as its own fee, lowering surplus. The protocol fee cancels out of ranking. Once BYOS's cut equals the gas cost, `surplus - gas` is the score the autopilot will compute for the bid. The scoring formula uses `quotedBuyAmount` from the proposal to compute surplus and clearing prices.
...
- **Slippage debit after settlement.** After a successful settlement, the penalty job computes the gap between `quotedBuyAmount` and the delivered amount. It converts this gap to ETH with the auction reference price. The job debits the sub-solver's escrow for the ETH-equivalent gap.
