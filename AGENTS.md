...
The integration tier builds both Hono apps in-process — it does not need a running service. Every DB-backed test gets its own database; `apps/byos/test/setup.ts` creates it and sweeps stale ones. The on-chain tier self-skips without `RUN_ONCHAIN_TESTS=1`, which is why CI does not run it. The e2e tier requires the full CoW Protocol stack running in Docker (`pnpm e2e:up`).
