...
describe("fee transform verification (COW-1240)", () => {
	it(
		"driver's fee application leaves Trampoline execute identical and only reduces buy amount",
		{ timeout: 180_000 },
		async () => {
			...
			const trampolineAddress = await publicClient.readContract({
				...
				abi: parseAbi([
					"function addressOf(address _subSolver) view returns (address _trampoline)",
				]),
			...
			const executeInteraction = intraInteractions.find(
				(i) =>
					(i.target as string).toLowerCase() ===
					(trampolineAddress as string).toLowerCase(),
			);
			...
			console.log("\nclearing price [sell]: ", driverSellPrice.toString(), "(driver set, index", trade.sellTokenIndex.toString() + ")");
			console.log("clearing price [buy]:  ", driverBuyPrice.toString(), "(driver set, index", trade.buyTokenIndex.toString() + ")");
			...
			console.log("✓ Invariant 3: credited buy reduced by exactly protocol fee; on-chain balance matches");
